"""
Minimal '.env' file loading for VMTrader.

VMTrader reads a handful of settings from the environment, most notably
VMTRADER_CSV_DATA_DIR. This module lets those be kept in a '.env' file
...
call 'load_env_file()' themselves, so importing VMTrader never reads or
modifies the environment as a side effect.

Variables already present in the environment always win, so an explicit
'export VMTRADER_CSV_DATA_DIR=...' is never overwritten by the file.

The '.env' file is searched for in the following order, and the first one
found is used:

1. The path in the VMTRADER_ENV_FILE environment variable, if set.
...
def find_env_file(start_dir=None):
    ...
    """
    explicit = os.environ.get('VMTRADER_ENV_FILE')
    if explicit:
