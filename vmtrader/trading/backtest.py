...
class BacktestTradingSession(TradingSession):
    ...
    def _create_data_handler(self, data_handler):
        ...
        try:
            os.environ['VMTRADER_CSV_DATA_DIR']
        except KeyError:
            if settings.PRINT_EVENTS:
                print(
                    "The VMTRADER_CSV_DATA_DIR environment variable has not been set. "
                    "This means that VMTrader will fall back to finding data within the "
                    "current directory where the backtest has been executed. However "
                    "it is strongly recommended that a VMTRADER_CSV_DATA_DIR environment "
        ...
        # TODO: Only equities are supported by VMTrader for now.
