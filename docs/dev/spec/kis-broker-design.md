...
포지션(`hldg_qty`)이 당일 매수분을 즉시 반영하는지는 **확인됐다 (2026-08-20 스모크 A-3)** — 15:22 접수·15:30 체결된 매수 1주가 같은 날 잔고 조회에 즉시 나타났고, 로컬 포트폴리오와 3종목 수량이 완전히 일치했다. 즉 **포지션 대조는 당일에도 유효하다.** 현금만 D+2 때문에 `prvs_rcdl_excc_amt`를 써야 한다.
...
| Q3 | ~~신호용 과거 시세를 어디서 얻는가?~~ **해소 (2026-08-20, 구현됨)** — [ADR-0012](../adr/0012-signal-history-from-venue.md): **브로커 일봉 API**(`inquire_daily_itemchartprice`, 모의계좌 동작·수정주가 지원·1회 100건)에서 받고, **매 기동마다 버퍼를 워밍업**한다 | 물음이 둘이었다 — 조달처뿐 아니라 **cron 단발(ADR-0009)이라 매 기동이 빈 버퍼에서 시작**한다는 것. 워밍업 없이는 200일 이동평균이 매일 당일 가격 하나로 계산된다. 이력 0인 종목이 있으면 그날은 거래하지 않는다 | 해소 |
...
| Q8 | ~~실전(`prod`) 승격 기준~~ **해소 (2026-08-20)** — [ADR-0013](../adr/0013-real-money-promotion-criteria.md): **자동 7항 + 수동 5항**. 자동분은 `scripts/promotion_check.py`가 모의 운용 원장을 읽어 판정한다 | 기준을 문서로만 두면 승격하고 싶은 순간에 읽히지 않는다. 원장이 증거이므로 별도 운용 일지가 필요 없다. 특히 **토큰 발급 주체**는 수동 항목이다 — KIS가 동일 앱키의 60초 내 재발급을 거부하므로 다른 봇과 앱키를 공유하면 본 엔진은 소비자여야 한다 | 해소 (통과 판정은 #27 이후) |
