...
> **후속 (2026-08-18, v0.3.13)**: §8-1(데이터 계층 계약 부재)은 v0.3.12에서, §8-7의 실행 알고리즘·옵티마이저 주입 불가는 v0.3.13에서 해소되었다. §6의 T5~T8은 수행 완료다. 아래 본문은 **작성 시점(`b94c6c0`, v0.3.10)의 기록**이며, 보고서는 스냅샷이므로 갱신하지 않는다. 테스트 수와 커버리지를 비롯한 수치는 그 시점의 것이다.
