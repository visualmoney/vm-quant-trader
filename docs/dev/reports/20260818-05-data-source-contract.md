...
| 상태 | **F3는 v0.3.13에서 수정됨** (작성 시점에는 미수정) |
| 근거 | [20260818-01](20260818-01-codebase-comprehension-strategy.md) §6의 **T8** (데이터 소스 교체) |
| 결함 번호 | `F3` — 보고서 04의 `F1`, `F2`에서 이어짐 |

> **후속 (2026-08-18, v0.3.13)**: §5.5의 수정안이 적용되어, 데이터 시작 이전 조회는 NaN을 반환하고 사이저의 가드가 정상 동작한다. 아래 본문은 **수정 이전 시점의 기록**이며, 보고서는 스냅샷이므로 갱신하지 않는다. 현재 코드의 동작은 CHANGELOG와 테스트를 참조할 것.
