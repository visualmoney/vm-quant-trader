...
- ~~cron 사이클의 배리어~~ — **구현됨**(결정 12, B3). 게시 → `executor.stop(timeout)` →
  ...
  스레드 쪽만 실패한다.
- **M1 — fill 핸드오프가 유실되면 자가복구되지 않는다.** `_poll_once`가 워터마크를 생산자
  쪽에서 전진시키므로, `_fill_buffer`를 유계 메일박스로 바꾸는 순간 버려진 체결이 다시
  방출되지 않는다. **이 배선 전에 워터마크를 소비자 쪽으로 옮겨야 한다.**
...
- **M6 — 훅 계약이 강제되지 않는다.** 조립부가 `BaseStrategy`가 아니라 `AlphaModel`을
  넘기므로, `OrderFilled`가 흐르기 시작하면 체결마다 `AttributeError`가 나고 루프가 삼킨다.
- **M2·M3 — 공유 인프라.** 게이트웨이의 `_CALL_LOCK`이 락을 쥔 채 sleep하므로 **느린 전략이
  주문 제출을 멈출 수 있고**, `data_handler`·`universe`·`signals`가 §3 소유권표에 없다.
