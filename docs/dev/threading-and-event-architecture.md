...
| # | 책임 | `settle()` 안의 어디 |
|---|---|---|
| 1 | 시간 예산 | 루프 머리의 `deadline` 비교 |
| 2 | 킬스위치 게이트 | 그 다음 줄의 `is_kill_switch_engaged()` |
| 3 | 라운드 구성 | `posted = list(self.open_orders)`와 뒤따르는 `post_task` |
| 4 | 드레인 배리어 + 하트비트 | `join_tasks(timeout=heartbeat)`와 그 뒤의 무제한 재시도 |
| 5 | 워커 수명 | 머리의 `TaskQueueWorker(...)`·`start()`와 `finally`의 `stop()` |
| 6 | 종결과 반환값 | `finally` 뒤의 `STALE` 루프와 `return booked` |
...
**Phase 1a — cron 모양을 유지한 채 스레드만 켠다** · **저장소 쪽 작업 완료**

전환 자체는 **배포의 조립부가 `LiveTradingSession(..., synchronous=False)`를 넘기는 것**
하나다. 세션의 기본값은 `True`로 **둔다** — 라이브러리가 대신 정하지 않고 조립부가 명시적으로
켜게 하는 것이 결정 4의 정신이고, 실물 계좌 경로([ADR-0013](adr/0013-real-money-promotion-criteria.md))에서
기본값 변경만으로 스레드가 켜지는 것은 위험하다.

**수용 기준은 통과했다** — `test_a_live_cycle_is_the_same_with_the_strategy_on_a_thread`가
실제 조립으로 사이클 전체(대조·결정·사이징·제출·정산·평가·기록)를 두 모드로 돌려 벤더에
도달한 것·원장·보유·현금·자산곡선이 모두 같음을 확인한다. Phase 0의 `.dat` 완전 일치에
대응하는 자리다. 배리어를 빼면 스레드 쪽만 실패한다.

- ~~시간 예산 산술(S14)~~ — **기록됨.** 네 숫자의 부등식을 `live.py`와 운영 문서 §3에.
  **기본값끼리는 성립하지 않으므로** 상한을 올리거나 `time_budget`을 줄여야 한다.
- ~~백테스트는 동기 유지(S15)~~ — **불변식으로 고정됨.** `BACKTEST_IS_SYNCHRONOUS`와
  조립에 대한 단언. 백테스트 루프에는 배리어를 걸 사이클 끝이 없고 `stats`가 클로저로
  이음매를 넘는다.
