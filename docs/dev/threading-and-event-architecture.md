...
## 4. `BaseStrategyExecutor` 설계 — 결정 13개 (v3)
...
- **M4 — 두 모드가 실패에서 비동형이다.** 동기 경로가 `_dispatch`를 무방비로 부르고
  `on_error`는 동기 조립에서 죽은 코드다. 단순히 양쪽을 삼키게 만드는 것은 **해법이
  아니다** — 데이터가 늦게 시작하는 백테스트의 시끄러운 거절이 조용한 오답이 된다(원칙 1).
  어떤 실패가 사이클을 끝내야 하는지의 어휘가 먼저다.
