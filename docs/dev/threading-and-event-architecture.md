...
**벤더 클라이언트가 가장 급하다.** 게이트웨이의 호출 스로틀은 **모듈 수준 락을 sleep 동안
쥔다**(`scripts/kis_gateway.py`). 두 액터가 같은 클라이언트를 쓰면 Actor 2의 가격 조회가
Actor 1의 `place_market_order`와 그 락을 놓고 경합하므로, **느린 전략이 주문 제출을 멈출 수
있다** — §3이 분리의 존재 이유로 내건 "전략이 5초 걸려도 브로커는 안 멈춘다"가 정면으로
깨진다. 인스턴스를 나누는 것으로는 부족하다(락이 모듈 수준이다). **Actor 2는 벤더를 호출하지
않아야 하고**, 그 수단은 리밸런스 직전에 Actor 1이 뜬 **가격 스냅샷**이다 — ADR-0006의
"사이저가 모든 목표를 하나의 스냅샷에서 만들었다"와 같은 논리이며 결정 10이 계좌 값에
대해 이미 내린 결론이다. 구현은 Phase 1a에 남아 있다(§7).
...
- ~~M3 — §3 소유권표가 공유 인프라를 빠뜨림~~ — **소유자 배정됨**(§3). 남은 것은 **배선**:
  Actor 2에게 가격 스냅샷을 주고 벤더 클라이언트를 그쪽에서 완전히 떼어낸다. 인스턴스를
  나누는 것으로는 부족하다 — `_CALL_LOCK`이 모듈 수준이라 두 인스턴스도 같은 락을 다툰다.
