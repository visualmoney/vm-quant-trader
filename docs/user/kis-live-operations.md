...
### 실행 시간 상한 — 프로세스 밖에서만 걸 수 있다

`flock -n`은 **겹침**을 막지 이미 도는 프로세스가 끝나는 것을 보장하지 않는다. 벤더 호출에 물린 메인 스레드를 자를 수 있는 것은 프로세스 밖의 상한뿐이다.

```cron
0 10 * * 1-5 flock -n /home/ec2-user/lock/vmtrader.lock \
  timeout -k 30 3600 /path/to/run_live.sh rebalance >> ... 2>&1
```

systemd 유닛을 쓴다면 `Type=oneshot`에서는 `RuntimeMaxSec=`이 무효이므로 **`TimeoutStartSec=`**를 쓴다.

**상한은 사이클의 나머지 예산 합보다 커야 한다.**

```
프로세스 상한  >  전처리 + 전략 예산 + (정산 마감 − 시작) + 워커 join
```

| 항목 | 기본값 | 어디서 |
| --- | --- | --- |
| 전처리 (대조 + 신호 워밍업) | 가변 | — |
| 전략 예산 | 60초 | `STRATEGY_BUDGET_SECONDS` |
| 정산 마감 | 최대 60분 (또는 마감 10분 전) | `time_budget`·`close_buffer` |
| 워커 join | 30초 | `settle(shutdown_timeout=)` |

**기본값끼리는 이 부등식이 성립하지 않는다** — 3600초 상한에 합이 3690초 + 전처리다. 그대로 두면 `SIGTERM`이 **정산 마무리 도중에** 떨어지고, 그 창은 아직 살아 있는 주문의 원장 기록을 잃는 자리다. 상한을 올리거나 `time_budget`을 줄인다.
...
파일을 쓴 것이 신호다. 프로세스가 항상 떠 있지 않으므로 시그널이나 소켓으로는 세션 사이에 멈출 수 없고, 파일이면 세션 중이든 사이든 동일하게 동작한다. 엔진은 기동 직후·주문 직전·정산 라운드마다 확인한다. 가장 오래 무시되는 시간은 왕복 1회가 아니라 **정산 라운드 1회**다 — 라운드는 미결 주문을 전부 조회한 뒤에야 다음 확인에 도달한다.

**멈추면 미체결 주문은 취소하지 않는다.** 취소도 주문이라, 종료 경로에서 실패하면 처리할 곳이 없다. 원장에 `SUBMITTED`로 남고 다음 기동의 대조가 브로커에 다시 물어 정리한다 — **`STALE`로 종결하지 않는다.** `STALE`은 종료 상태라 다음 기동이 조회 대상에서 빼기 때문이다. `STALE`은 킬스위치가 아니라 **시간 예산이 만료됐을 때**, 즉 엔진이 스스로 포기했을 때만 붙는다.
...
| 미체결 — 예산 만료 | 원장 `orders`의 `STALE` | 있으면 다음 리밸런싱이 흡수 |
| 미체결 — 킬스위치로 중단 | 원장 `orders`의 `SUBMITTED` | 다음 기동의 대조가 브로커에 다시 물어 정산 |
