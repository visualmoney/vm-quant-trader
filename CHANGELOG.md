...
* Separates the live plane into a venue-neutral half and a vendor half, and takes the vendor name out of the neutral one. Five of the seven modules under 'broker/kis/' knew nothing about KIS: the BrokerClient Protocol and its records, the FIFO worker, the safety guard, the SQLite ledger and reconciliation. Only the response parser did. They move to 'vmtrader/broker/live/' and the parser stays behind. This is a breaking change; see ADR-0015.
* Renames KisBroker to LiveBroker and moves it to 'vmtrader/broker/live_broker.py'. The class never imported KIS: across 877 lines the only thing that knew which broker it was talking to was the string 'kis broker: ' in a log line. Everything else went through the Protocol. The log prefix is now a 'venue_name' constructor argument, and the 'account_id' default changes from 'kis' to 'live'; the KIS assembly sites pass both explicitly, so operator-visible naming is unchanged. account_id identifies the local Portfolio and is not written to the ledger, so accounting is unaffected.
* Adds venue-neutral exceptions, because LiveDataHandler raised the KIS parser's own exception for a price it could not obtain. A second venue's price failure would have been reported as a KIS error. It now raises PriceUnavailable, and KisParseError subclasses the new VenueParseError, which itself inherits ValueError, so every existing 'except KisParseError' keeps working and a second venue's parser has a base to subclass.
* Removes the three imports that reached from the engine core into the vendor package: two in 'trading/live.py' and one in 'data/live_data_handler.py'. A grep for 'broker.kis' outside 'broker/kis/' now returns nothing.
* Adds tests/unit/test_vendor_import_boundary.py, which reads the package with ast rather than importing it and asserts that no module imports a broker SDK, that vendor code is imported only from within its own package, and that 'broker/live/' names no vendor at all. The spec has asked for this check since it was written and nothing performed it. Verified against an injected violation, not only against the passing state.
* No behaviour changes. The suite runs the same 514 cases with the same results before and after the move, which is what makes it a move; the boundary check adds 201 more, for 715.
* Drops the four funding methods from the Broker ABC, taking it from thirteen abstract methods to nine. They were meaningful only in the simulator: a backtest has to start with money from somewhere, so SimulatedBroker moves cash between its master account and its sub-portfolios. No venue offers a transfer API, so LiveBroker implemented all four purely to raise, which meant the interface promised something every live implementation had to refuse. Funding is now part of SimulatedBroker rather than of Broker, and the class docstring no longer describes the master account as the route for subscriptions and withdrawals. See ADR-0016.
* Deletes those four refusals and their shared helper from LiveBroker rather than keeping them as documented errors. They existed because the ABC forced them to; with the requirement gone, an AttributeError is the more honest answer, since a live account will not gain a deposit API later. The operational advice the message carried, that funding happens at the broker and the engine reads the balance, is already in the operations guide and in seed_from_venue's docstring.
* Pins the removal from both sides: the ABC test asserts the four are absent from Broker.__abstractmethods__ and present on SimulatedBroker, and the live broker test asserts they are absent from the instance. The second matters because these are easy to reintroduce by copying from the simulator.
* Declines to add a paper broker, and makes paper a mode instead. A broker's paper account is a real venue, not a simulator: order acceptance, partial fills, rejections and rate limits all happen on the same endpoints in the same shapes. That the rehearsal runs the same code path as real money is the whole of its value, and the promotion criteria in ADR-0013 rest on it, so splitting the class would leave the rehearsal proving nothing. See ADR-0017.
* Adds 'venue' and 'mode' to the BrokerClient Protocol, and derives the gateway's mode from the server it authenticated against rather than taking it as a separate argument, so it cannot disagree with the account actually dialled. The engine cannot work this out for itself, because a paper account answers identically.
* Gives the ledger the identity of the deployment that wrote it, and makes it refuse a different one. promotion_check.py said in its own docstring that it read 'the ledger a paper deployment produced' and then read whichever file it was handed: a real-money ledger, or one with both mixed in, produced the same verdicts. A ledger now records venue, mode and account, raises LedgerIdentityConflict when another deployment stamps it, and stores the mode on each equity row as well, so a file assembled before this existed can still be audited. That row default is 'unknown' rather than 'paper', since a lenient default would pass every pre-existing ledger for free, which is the trust being replaced. Existing ledgers gain the column through an ALTER on open.
* Adds 'ledger-is-paper' as the first promotion criterion. It fails for a ledger that records no identity, for one belonging to a real deployment, and for one carrying foreign rows, and names the venue and account when it passes. Everything below it measures how a deployment behaved, which means nothing if the deployment was not the one being promoted.
* Stops charging securities transaction tax on ETF sales. A domestic ETF is a beneficiary certificate rather than a share and its sale is outside the tax entirely, but KoreaStockFeeModel decided on the sign of the quantity alone, so examples/sixty_forty_kr.py was charging 0.18% on every sale of KODEX 200 and KOSEF. For a portfolio built only from ETFs that is not a slightly wrong rate, it is the whole of a tax that is not owed, collected at every monthly rebalance. The engine cannot tell an ETF from a share -- both are six digits -- so the caller names them through a new 'tax_exempt_assets' argument, which keeps one model usable for a universe mixing the two. Nothing is exempt by default. Verify the rates against your own broker's schedule; these are the statutory shape rather than a price list.
