...
def default_output_dir():
    ...
    'os.environ.get('QSTRADER_CSV_DATA_DIR', '.')'.
    ...
    """
    return os.environ.get('QSTRADER_CSV_DATA_DIR', '.')
...
def download_to_csv(symbol, output_dir, period='max', start=None, end=None):
    ...
    QSTrader-compatible CSV file within the output directory.
...
def parse_args(argv=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            'Yahoo! Finance에서 일봉 OHLCV 데이터를 받아 QSTrader가 읽을 수 있는 '
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(기본값: 환경변수 QSTRADER_CSV_DATA_DIR 또는 '.env'의 값, 없으면 현재 디렉토리)"
        )
    )
    parser.add_argument(
        '-p', '--period',
